<script>
	import { ChevronDown, Minus, Plus, SlidersHorizontal, MapPin } from "lucide-svelte";

    let selected = false;
</script>

<main class=" w-screen px-4 max-w-[400px]">
    ...
    <div class=" mt-3 flex justify-between w-full px-2">
        ...
        <div class=" flex mt-2">
            <button class=" w-9 h-9 bg-mainPurple rounded-md flex justify-center items-center mx-[0.13rem]"><SlidersHorizontal color="#ffffff" /></button>
            <button class=" w-9 h-9 bg-mainPurple rounded-md flex justify-center items-center mx-[0.13rem]"><MapPin color="#ffffff"/></button>
        </div>
    </div>
    <div class=" mt-6">
        <div class=" relative h-20 flex items-center justify-between border border-gray-400 rounded-xl mt-5">
            <img class=" w-7 absolute right-[-10px] top-[-15px]" src="/favicon.png" alt="">
            <div class=" flex h-full items-center">
                <img class=" w-16" src="/favicon.png" alt="">
                <div>
                    <p class=" font-medium text-sm">Sjokolade melk Q 1L</p>
                    <p class=" text-xs text-gray-400">Q-meieriene</p>
                    <p class=" font-medium text-sm">29.90 kr</p>
                </div>
            </div>
            <div class=" h-full flex items-center mr-4">
                <div class=" border border-mainPurple rounded-md h-9 flex w-28 justify-between items-center">
                    <button class=" w-10 flex justify-center items-center"><Minus size="15px"/></button>
                    <p>1</p>
                    <button class=" w-10 flex justify-center items-center"><Plus size="15px"/></button>
                </div>
            </div>
        </div>
    </div>
